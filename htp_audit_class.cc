    ...
    MYSQL_THD thd __attribute__((unused)), unsigned int event_class, const void *event)
{
  ...
  event_info_t info;
  char user[256];
  htp_audit_general_event_get_user(event_general->general_user, user);
  ...
  info.command = event_general->general_command;
  info.query = event_general->general_query;
    ...
    MYSQL_THD thd __attribute__((unused)), unsigned int event_class, const void *event)
{
  ...
  event_info_t info;
  info.ip = event_connection->ip;
  info.user = event_connection->user;
  info.host = event_connection->host;
